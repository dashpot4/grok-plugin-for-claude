...
function inferLegacyJobPhase(job, progressPreview = []) {
  ...
  for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
    ...
    if (line.startsWith("starting grok")) {
      return "starting";
    }
    if (line.startsWith("grok finished")) {
      return "finalizing";
    }
    if (line.startsWith("grok failed")) {
      return "failed";
    }
  }
...
export function findLatestResumableTaskJob(jobs) {
  return jobs.find((job) => job.jobClass === "task" && job.threadId && job.status === "completed") ?? null;
}