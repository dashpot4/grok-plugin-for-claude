...
argument-hint: '[--wait|--background] [--no-web|--web] [--effort <low|medium|high|xhigh>] [--base <ref>] [--scope auto|working-tree|branch]'
...
- However, scan for natural language effort requests (e.g. "grok max 모드", "use high effort") and ensure `--effort <level>` is included in the arguments passed to the companion (`max` intent maps to `xhigh`) so the workspace default can be overridden. Explicit flags take precedence.
